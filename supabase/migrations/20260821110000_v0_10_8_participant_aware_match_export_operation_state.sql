-- v0.10.8: allow participant-aware match export operation success metadata.

alter table public.export_operations
    drop constraint export_operations_rows_written_check;

alter table public.export_operations
    add constraint export_operations_rows_written_check
    check (rows_written is null or rows_written between 1 and 12);
...
begin
    ...
    if (
        ...
        and (p_rows_written is null or p_rows_written not between 1 and 12)
    ) or (
        v_operation_type = 'export_standings'
        and (p_rows_written is null or p_rows_written <> 12)
    ) then
