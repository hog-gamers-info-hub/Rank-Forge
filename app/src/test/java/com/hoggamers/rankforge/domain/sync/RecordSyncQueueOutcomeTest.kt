...
class RecordSyncQueueOutcomeTest {
    @Test fun completedDoesNotCreatePendingEntryAndFailuresPersistDeterministically() = runTest {
        ...
        recorder.record(OWNER_A, SyncQueueOperationType.TOURNAMENT_UPLOAD, "tournament", SyncQueueStatus.COMPLETED)
        assertTrue(repository.entries.isEmpty())
        listOf(SyncQueueStatus.BLOCKED_AUTHENTICATION, SyncQueueStatus.BLOCKED_NETWORK, SyncQueueStatus.FAILED_VALIDATION, SyncQueueStatus.FAILED_AUTHORIZATION, SyncQueueStatus.FAILED_LOCAL, SyncQueueStatus.FAILED_UNKNOWN).forEach { status -> recorder.record(OWNER_A, SyncQueueOperationType.MATCH_RESTORATION, "tournament", status) }
        ...
        recorder.record(OWNER_A, SyncQueueOperationType.MATCH_RESTORATION, "tournament", SyncQueueStatus.COMPLETED)
        assertEquals(1, repository.entries.size)
    ...
    private class RecordingRepository : PersistentSyncQueueRepository {
        ...
        override suspend fun enqueue(ownerUserId: String, operationType: SyncQueueOperationType, tournamentId: String?, status: SyncQueueStatus, failureCategory: String?): SyncQueueEntry {
            val existing = entries.indexOfFirst { it.ownerUserId == ownerUserId && it.operationType == operationType && it.tournamentId == tournamentId && it.status != SyncQueueStatus.COMPLETED }
            if (existing >= 0) {
                entries[existing] = entries[existing].copy(status = status, failureCategory = failureCategory)
                return entries[existing]
            }
            return SyncQueueEntry("${entries.size}", operationType, tournamentId, 0, status, failureCategory, 0, ownerUserId).also(entries::add)
        }
