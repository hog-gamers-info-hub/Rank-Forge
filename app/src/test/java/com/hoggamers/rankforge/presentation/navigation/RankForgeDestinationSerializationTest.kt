...
class RankForgeDestinationSerializationTest {
    ...
    @Test
    fun authDestinationSurvivesSerializationRoundTrip() {
        val encoded = json.encodeToString(AuthDestination)
        val decoded = json.decodeFromString<AuthDestination>(encoded)

        assertEquals(AuthDestination, decoded)
    }
    ...
    @Test
    fun matchKillDestinationSurvivesSerializationRoundTrip() {
        val destination = MatchKillDestination(
            tournamentId = "tournament-id",
            matchId = "match-id",
        )

        val encoded = json.encodeToString(destination)
        val decoded = json.decodeFromString<MatchKillDestination>(encoded)

        assertEquals(destination, decoded)
    }

    @Test
    fun matchReviewDestinationSurvivesSerializationRoundTrip() {
        val destination = MatchReviewDestination(
            tournamentId = "tournament-id",
            matchId = "match-id",
        )

        val encoded = json.encodeToString(destination)
        val decoded = json.decodeFromString<MatchReviewDestination>(encoded)

        assertEquals(destination, decoded)
    }

    @Test
    fun matchOcrReviewDestinationSurvivesSerializationRoundTrip() {
        val destination = MatchOcrReviewDestination(
            tournamentId = "tournament-id",
            matchId = "match-id",
        )

        val encoded = json.encodeToString(destination)
        val decoded = json.decodeFromString<MatchOcrReviewDestination>(encoded)
