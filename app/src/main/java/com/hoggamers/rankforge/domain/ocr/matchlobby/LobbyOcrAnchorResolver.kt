...
class LobbyOcrAnchorResolver {
    ...
    fun resolve(
        ...
        observations: List<LobbyOcrAnchorObservation>,
        imageDimensions: OcrImageDimensions,
    ): List<LobbyResolvedOcrAnchor> = resolveGroup(screenshotIndex, observations, imageDimensions).anchors
    ...
    private fun resolveGroup(
        ...
        val expectedSlots = expectedSlotsFor(screenshotIndex) ?: return LobbyResolvedOcrAnchorGroup(
        ...
        val best = resolveBestAssignment(expectedSlots, candidatesBySlot)
    ...
    private fun resolveBestAssignment(
        ...
        fun visit(index: Int) {
            if (index == expectedSlots.size) {
                ...
                if (assignment.isOrderingValid() && assignment.isBetterThan(best)) {
                    best = assignment
                }
                return
            }
    ...
    private data class Assignment(
        ...
        private fun ordered(first: Int, second: Int, horizontal: Boolean): Boolean {
        ...
        private fun pairError(first: Int, second: Int, horizontal: Boolean): Double {
